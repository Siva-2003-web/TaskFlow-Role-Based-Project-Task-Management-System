...
const EmployeeTasks = () => {
  ...
  const filtered = tasks.filter((t) => {
    ...
    const matchProject =
      projectFilter === "all" ||
      (t.project?._id || t.project) === projectFilter;
  ...
  const getProjectName = (pid) =>
    projects.find((p) => p._id === pid)?.name || "Unassigned Project";
  ...
  const statusDotClass = (s) => {
    const map = {
      pending: "et-dot--pending",
      "in-progress": "et-dot--progress",
      review: "et-dot--review",
      completed: "et-dot--completed",
    };
    return `et-dot ${map[s] || map.pending}`;
  };

  const priorityBadge = (p) => (
    <span className={`mt-priority mt-priority--${p || "medium"}`}>
      {p || "medium"}
    </span>
  );
  ...
  return (
    <div className="et-page animate-fade-in">
      ...
      <div className="et-quick-stats">
        <div className="et-qs">
          <span className="et-qs__val">{stats.total}</span>
          <span className="et-qs__lbl">Total</span>
        </div>
        <div className="et-qs et-qs--pending">
          <span className="et-qs__val">{stats.pending}</span>
          <span className="et-qs__lbl">Pending</span>
        </div>
        <div className="et-qs et-qs--progress">
          <span className="et-qs__val">{stats.inProgress}</span>
          <span className="et-qs__lbl">In Progress</span>
        </div>
        <div className="et-qs et-qs--done">
          <span className="et-qs__val">{stats.completed}</span>
          <span className="et-qs__lbl">Completed</span>
        </div>
      </div>
          ...
          {Object.entries(grouped).map(([pid, pTasks]) => (
            <div className="et-group" key={pid}>
              <div className="et-group__header">
                ...
                <span className="et-group__count">
              ...
              <div className="et-task-list">
                {pTasks.map((t, idx) => {
                  ...
                  return (
                    <div
                      className={`et-task ${isUpdating ? "et-task--updating" : ""}`}
                      key={t._id || idx}
                    >
                      <div className="et-task__left">
                        ...
                        <div className="et-task__info">
                          ...
                          {t.description && (
                            <p className="et-task__desc">{t.description}</p>
                          )}
                          {t.dueDate && (
                            <p
                              className="et-task__due"
                              style={{
                                fontSize: "0.6875rem",
